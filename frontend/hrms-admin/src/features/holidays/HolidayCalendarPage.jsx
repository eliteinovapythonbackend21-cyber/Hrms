...
function CalendarLegend({ label, tone }) {
  const styles = {
    violet:
...
/* ============================================================
   MAIN PAGE — CALENDAR + GOVERNMENT / SUNDAY SYNC
   The plain holidays list now lives on the separate "Holidays"
   screen (HolidayListPage.jsx).
============================================================ */
...
export default function HolidayCalendarPage() {
  ...
  const { canAdd, canEdit } = useModulePermissions("Holidays");
  const addMagnet = useMagnetic(0.25);

  const createHoliday = useCreateHoliday();
  const updateHoliday = useUpdateHoliday();
  const syncGovernmentHolidays = useSyncGovernmentHolidays();
  const unsyncGovernmentHolidays = useUnsyncGovernmentHolidays();
  const syncOfficeSundays = useSyncOfficeSundays();

  const { data: allHolidaysData, refetch } = useHolidays({
    page: 1,
    per_page: 1000,
  });
  ...
  const allActiveHolidays = useMemo(
    () => allHolidays.filter((holiday) => holiday.is_active),
    [allHolidays]
  );

  const allInactiveHolidays = useMemo(
    () => allHolidays.filter((holiday) => !holiday.is_active),
    [allHolidays]
  );
  ...
  const [unsyncConfirmOpen, setUnsyncConfirmOpen] = useState(false);
  ...
  const [sundayStartYear, setSundayStartYear] = useState(
    new Date().getFullYear()
  );
  const [sundayEndYear, setSundayEndYear] = useState(new Date().getFullYear());
  ...
  const availableYears = useMemo(() => {
    ...
    years.add(Number(syncYear));
    years.add(Number(sundayStartYear));
    years.add(Number(sundayEndYear));
    years.add(Number(listYear));
  ...
  const holidaysByDate = useMemo(() => {
    ...
    allActiveHolidays.forEach((holiday) => {
      const key = holidayDateKey(holiday.holiday_date);
      if (!key) return;
  ...
  const calendarHolidaysThisMonth = useMemo(
    () =>
      allActiveHolidays.filter((holiday) => {
        const parsed = parseHolidayDate(holiday.holiday_date);
        if (!parsed) return false;
        return parsed.year === calendarYear && parsed.month === calendarMonth;
      }),
  ...
  const calendarCells = useMemo(() => {
    ...
    for (let day = 1; day <= daysInMonth; day += 1) {
      const key = getCalendarDateKey(calendarYear, calendarMonth, day);
    ...
    while (cells.length < 42) {
      ...
      const year = date.getFullYear();
      const month = date.getMonth();
      const key = getCalendarDateKey(year, month, nextDay);
    ...
    return cells;
  }, [calendarYear, calendarMonth, holidaysByDate]);
  ...
  const handleCalendarCellClick = (cell) => {
    ...
    if (cell.holidays.length > 0) return;
    if (!canAdd) return;
  ...
  const handleSyncGovernmentHolidays = async () => {
    try {
      ...
      setCalendarYear(Number(syncYear));
      setCalendarMonth(0);
      setListYear(Number(syncYear));
  ...
  const handleUnsyncGovernmentHolidays = async () => {
    try {
      const result = await unsyncGovernmentHolidays.mutateAsync({
        year: syncYear,
        countryCode: syncCountry,
      });
  ...
  const handleSyncOfficeSundays = async () => {
    try {
      const startYear = Number(sundayStartYear);
      const endYear = Number(sundayEndYear);

      if (!startYear || !endYear) {
        showToast("Please select a valid year range", "error");
        return;
      }

      if (endYear < startYear) {
        showToast("End year cannot be smaller than start year", "error");
        return;
      }

      const result = await syncOfficeSundays.mutateAsync({
        startYear,
        endYear,
      });
  ...
  const groupedByMonth = useMemo(() => {
    ...
    allActiveHolidays.forEach((holiday) => {
      const parsed = parseHolidayDate(holiday.holiday_date);
      if (!parsed) return;
      if (parsed.year !== Number(listYear)) return;

      if (getHolidayType(holiday) === "Government") {
        buckets[parsed.month].government.push(holiday);
      } else {
        buckets[parsed.month].office.push(holiday);
      }
    });

    buckets.forEach((bucket) => {
      bucket.government.sort((a, b) =>
        holidayDateKey(a.holiday_date).localeCompare(
          holidayDateKey(b.holiday_date)
        )
      );

      bucket.office.sort((a, b) =>
        holidayDateKey(a.holiday_date).localeCompare(
          holidayDateKey(b.holiday_date)
        )
      );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="u-rise relative flex flex-col gap-4 overflow-hidden rounded-2xl border border-slate-200/80 bg-gradient-to-br from-white via-primary-50/40 to-white p-4 shadow-sm dark:border-white/[0.08] dark:from-primary-500/[0.08] dark:via-white/[0.02] dark:to-transparent xl:flex-row xl:items-center xl:justify-between">
        <GridPattern id="holiday-calendar-grid" />
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">
                Sync government holidays, add Sundays, and manage the calendar
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              type="button"
              variant="secondary"
              onClick={() => setUnsyncConfirmOpen(true)}
              disabled={
                syncGovernmentHolidays.isPending ||
                unsyncGovernmentHolidays.isPending ||
                syncOfficeSundays.isPending
            ...
            >
              {unsyncGovernmentHolidays.isPending ? "Removing..." : "Unsync"}
            </Button>
          </div>
        </div>
          ...
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <input
                ...
                min="1900"
                max="2100"
                value={sundayStartYear}
                onChange={(event) =>
                  setSundayStartYear(Number(event.target.value))
                }
              ...
              <input
                ...
                value={sundayEndYear}
                onChange={(event) =>
                  setSundayEndYear(Number(event.target.value))
                }
                className="h-9 w-24 rounded-lg border border-slate-300 bg-white px-2 text-xs outline-none focus:border-sky-400 dark:border-slate-600 dark:bg-white/[0.06] dark:text-white"
              />
              ...
              <Button
                type="button"
                variant="secondary"
                onClick={handleSyncOfficeSundays}
                disabled={
                  syncOfficeSundays.isPending ||
                  syncGovernmentHolidays.isPending ||
                  unsyncGovernmentHolidays.isPending
        ...
        <div className="overflow-visible rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
          <div className="border-b border-slate-200 bg-gradient-to-r from-slate-50 via-white to-violet-50/60 px-4 py-3 dark:border-white/10 dark:from-slate-800/80 dark:via-slate-900 dark:to-violet-950/20">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <CalendarLegend label="Government" tone="violet" />
                <CalendarLegend label="Office" tone="sky" />
                ...
                <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-semibold text-slate-500 dark:bg-white/[0.06] dark:text-slate-400">
                  ...
                  {calendarHolidaysThisMonth.length === 1 ? "" : "s"}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-7 gap-px bg-slate-200/70 dark:bg-slate-700/70">
            {calendarCells.map((cell) => {
              ...
              const dateObject = new Date(cell.year, cell.month, cell.day);
              const dayOfWeek = dateObject.getDay();
              const isSunday = dayOfWeek === 0;
              const isWeekend = dayOfWeek === 0 || dayOfWeek === 6;
              const hasHoliday = cell.holidays.length > 0;

              const governmentCount = cell.holidays.filter(
                (holiday) => getHolidayType(holiday) === "Government"
              ).length;

              const officeCount = cell.holidays.filter(
                (holiday) => getHolidayType(holiday) === "Office"
              ).length;
              ...
              return (
                <div
                  ...
                  className={`group relative min-h-[118px] overflow-visible p-1.5 transition ${
                    ...
                    cell.isCurrentMonth && !hasHoliday && canAdd
                ...
                >
                  ...
                  <div className="mt-1.5 space-y-1">
                    {cell.holidays.slice(0, 2).map((holiday) => {
                      const isGovernment =
                        getHolidayType(holiday) === "Government";

                      const isSundayOffice =
                        !isGovernment &&
                        holiday.name?.trim().toLowerCase() === "sunday";
                    ...
                    <div className="mb-2 flex items-center justify-between gap-2">
                      <div>
                        ...
                        <p className="mt-0.5 text-[8px] text-slate-400">
                          {cell.holidays.length} holiday
                          {cell.holidays.length === 1 ? "" : "s"}
                        </p>
                      </div>
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="mt-0.5 text-[8px] text-slate-400">
                                  {getHolidayType(holiday)} Holiday
                                </p>
                              </div>
                            </div>
                          );
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
          <div className="flex flex-col gap-3 border-b border-slate-200 px-4 py-3 dark:border-white/10 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                {yearTotals.government} Government · {yearTotals.office} Office
              </p>
            </div>

            <select
              value={listYear}
              onChange={(event) => setListYear(Number(event.target.value))}
          ...
          <div className="grid grid-cols-1 gap-3 p-3 md:grid-cols-2 xl:grid-cols-3">
            {MONTH_NAMES.map((monthName, monthIndex) => {
              ...
              const monthHolidays = [...bucket.government, ...bucket.office];
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-[8px] text-slate-400">
                              {isGovernment ? "Government" : "Office"}
                              {" • "}
                              {parsed?.day || "-"}
                            </p>
                          </div>
      ...
      >
        <HolidayForm
          initialData={
            editing || (prefillDate ? { holiday_date: prefillDate } : {})
          }
      ...
      <ConfirmDialog
        open={unsyncConfirmOpen}
        onClose={() => setUnsyncConfirmOpen(false)}
        onConfirm={handleUnsyncGovernmentHolidays}
        title="Unsync Government Holidays"
        message={`Are you sure you want to remove all Government holidays for ${syncYear}? This will remove the saved Government holiday records for the selected year.`}
        confirmText="Unsync Holidays"
        loading={unsyncGovernmentHolidays.isPending}
      />
    </div>
  );
}
