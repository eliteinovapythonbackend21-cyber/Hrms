...
export default function DesignationListPage() {
  ...
  const { data: departmentData } = useQuery({
    ...
    queryFn: async () => (await masterApi.listDepartments({ branch_id: branchFilterId, page: 1, per_page: 100, is_active: true })).data.data,
  ...
  const queryParams = { page, search, department_id: departmentFilterId || undefined };
  ...
  const inactiveDesignations = designations.filter((designation) => !designation.is_active);

  const filteredDesignations = designations.filter((designation) => {
    if (statusFilter === "active") return designation.is_active;
    if (statusFilter === "inactive") return !designation.is_active;
    return true;
  });

  const getDepartment = (designation) => designation?.department || designation?.department_details || null;

  const getCompany = (designation) => {
    const department = getDepartment(designation);
    return designation?.company || designation?.company_details || department?.company || department?.company_details || null;
  };

  const getBranch = (designation) => {
    const department = getDepartment(designation);
    const company = getCompany(designation);
    return designation?.branch || designation?.branch_details || department?.branch || department?.branch_details || company?.branch || company?.branch_details || null;
  };

  const getCompanyName = (designation) => getCompany(designation)?.name || designation?.company_name || designation?.company?.name || "-";
  const getBranchName = (designation) => getBranch(designation)?.name || designation?.branch_name || designation?.branch?.name || "-";
  const getDepartmentName = (designation) => {
    const department = getDepartment(designation);
    return department?.department_name || department?.name || designation?.department_name || "-";
  };
  const getDepartmentCode = (designation) => {
    const department = getDepartment(designation);
    return department?.department_code || designation?.department_code || "-";
  };
  const getEmployeeCount = (designation) => {
    if (Array.isArray(designation?.employees)) return designation.employees.length;
    if (typeof designation?.employee_count === "number") return designation.employee_count;
    return 0;
  };

  const handleAdd = () => { setSelectedDesignation(null); setModalOpen(true); };
  const handleEdit = (designation) => { setSelectedDesignation(designation); setModalOpen(true); };
  const handleCloseModal = () => { setModalOpen(false); setSelectedDesignation(null); };
  ...
  const handleDelete = (designation) => { setDesignationToDelete(designation); setDeleteOpen(true); };

  const confirmDelete = async () => {
    if (!designationToDelete) return;
    try {
      await deactivateDesignation.mutateAsync(designationToDelete.id);
      ...
      if (err.response?.status === 409) {
        setDeleteOpen(false);
        setBlockedInfo({ name: designationToDelete?.designation_name, message: err.response?.data?.message });
        return;
      }
      showToast(err.response?.data?.message || "Operation failed", "error");
    }
  ...
  const handleReactivate = async (designation) => {
    try {
      await updateDesignation.mutateAsync({ id: designation.id, payload: { is_active: true } });
      showToast("Designation reactivated successfully", "success");
      refetch();
    } catch (err) {
      showToast(err.response?.data?.message || "Operation failed", "error");
    }
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">Manage designations across companies, branches and departments</p>
          </div>
        </div>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filteredDesignations.map((designation) => {
                ...
                return (
                  <tr key={designation.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                          <p className="truncate text-sm font-medium text-slate-800 dark:text-slate-100">{designation.designation_name || "Unnamed Designation"}</p>
                          <p className="font-mono text-[10px] text-slate-400">{designation.designation_code || "-"}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-300">{getCompanyName(designation)}</td>
                    <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-300">{getBranchName(designation)}</td>
                    <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-300">{getDepartmentName(designation)}</td>
                    <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-300">{getEmployeeCount(designation)}</td>
                    <td className="px-4 py-3">{statusBadge(designation.is_active)}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        <button type="button" onClick={() => handleEdit(designation)} className="rounded-full p-1.5 text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-700" title="Edit">
                        ...
                        {designation.is_active ? (
                          <button type="button" onClick={() => handleDelete(designation)} className="rounded-full p-1.5 text-red-500 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-500/10" title="Deactivate">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          ...
                          <button type="button" onClick={() => handleReactivate(designation)} disabled={updateDesignation.isPending} className="rounded-full p-1.5 text-emerald-600 hover:bg-emerald-50 disabled:opacity-40 dark:text-emerald-400 dark:hover:bg-emerald-500/10" title="Reactivate">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {!isLoading && filteredDesignations.length === 0 && (
        <div className="flex min-h-[260px] flex-col items-center justify-center rounded-xl border border-slate-200 bg-white px-6 py-10 text-center shadow-sm dark:border-slate-700 dark:bg-slate-900">
          ...
          <p className="mt-1 max-w-sm text-xs text-slate-500 dark:text-slate-400">No designations match your current search, company, branch, department, or status filter.</p>
          <Button onClick={handleAdd} className="mt-4 h-9 px-4 text-sm">+ Add Designation</Button>
        </div>
      )}
      ...
      {/* MODALS */}
      <Modal open={modalOpen} onClose={handleCloseModal} title={selectedDesignation ? "Edit Designation" : "Add Designation"}>
        <DesignationForm
      ...
      <ConfirmDialog
        open={deleteOpen}
        onClose={() => { setDeleteOpen(false); setDesignationToDelete(null); }}
        onConfirm={confirmDelete}
        title="Deactivate Designation"
        message={designationToDelete ? `Are you sure you want to deactivate "${designationToDelete.designation_name}"?` : "Are you sure you want to deactivate this designation?"}
        confirmText="Deactivate"
        loading={deactivateDesignation.isPending}
      />
      ...
      <ConfirmDialog
        ...
        message={blockedInfo?.message || (blockedInfo?.name ? `"${blockedInfo.name}" cannot be deactivated because it still has active employees or related records.` : "This designation cannot be deactivated because it still has active records linked to it.")}
